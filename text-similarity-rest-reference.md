...
<p class="rtejustify">
  Before you start, please ensure that you have a <a href="/api-key">valid API key</a>.
</p>
...
<span style="color: #ff0000;"><strong>Points to note:</strong></span> * There is 

**no maximum length** for the text, but a 1MB maximum payload per request. * The text can be in **any language** - The text that you provide can be: -- plain text, (e.g. the cow jumps over the moon) -- text with POS annotations (e.g. *the/dt cow/nn jumps/vb*) -- manipulated texts such as n-grams (e.g. *thiscat catis iscute*). * Since this is a json request, your text has to be properly escaped and encoded in UTF-8</p> 
